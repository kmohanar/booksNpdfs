...
alfabank.use(jsonParser);
alfabank.use(urlencodedParser);
...
alfabank.post('/bg-validate', function (req, res) {
	var bg = req.body;
...
/* 
    ...
    if (commissionClaimed_t3 == true) {
        commissionClaimed_t3 = 'YES';
        console.log("commissionClaimed_t3 inside if=====>>>", commissionClaimed_t3);
    }
    console.log("commissionClaimed_t3", commissionClaimed_t3);
    ...
    //var response = res.send(ID + " has been Issued successfully");
    ...
    console.log("Before OpenBankGuarantee RESPONSE  ");
    chaincode.invoke.OpenBankGuarantee([ID, input], function (err, resp) {
    	console.log("RESPONSE  ", resp);
    	//var response = res.end(toString(resp));
    	console.log("response", resp);
    	var response = res.end(ID + " has been Issued successfully");
    ...
    request(requestOptions, function (error, response, body) {
        ...
        parseString(xml, function (err, result) {
            ...
            console.log("test------>>>>>>>>", test);
            console.log("error messages====>>>>>", test['S:Envelope']['S:Body'][0]['ns4:BgIssueResponse'][0]['Status'][0]);
            //['S:Body'][0]['ns4:BgIssueResponse'][0]['Status'][0]['messages']
            ...
            if (errorMSg == 'T24Error') {
                ...
                applicantbank.query("UPDATE bankguarantee SET status ='ISSUED' WHERE bgReqID = ?", [BGREQ], function (err, result) {  
                    if (err) throw err;
...
alfabank.get('/customer-lc-orders/:custName', function (getreqang, getresang) {
	...
	console.log("before customer-lc-orders ===>",getresang);
	chaincode.query.GetCustomerBasedRecords([customerName], function (err, resp) {
		console.log("customer-lc-orders ===>",resp);
		if (resp == null && resp == undefined)
		{ 
			console.log("inside if case");
			getresang.json({});
			...
			console.log("inside customer-lc-orders ===>",resp);
			var sTemp = "";
			var aObjs = [];
			var stop = 0;
			for (var i = 0; i < resp.length; ++i) {
...
alfabank.get('/customer-bg-orders/:custName', function (getreqang, getresang) {
	customerName = getreqang.params.custName
	console.log("before customer-bg-orders ===>",getresang);
	chaincode.query.GetCustomerBasedBgRecords([customerName], function (err, resp) {
		console.log("customer-bg-orders ===>",resp);
		if (resp == null && resp == undefined)
		{ 
			console.log("inside if case");
			getresang.json({});
...
alfabank.post('/bgamendreq', function (postreqang, postresang) {
	...
	var bgAmendRecord = [{
		"bgAmendId": uidata.bgNumber_t1,
		"bgAmendReqId": uidata.bgAmendReqId,
		"numberOfAmendment": uidata.bgNumberOfAmendments,
		"bgAmendPrincipalAmount": uidata.amount_t1,
		"bgAmendExpiryDate": uidata.expiryDate_t1,
		"bgTermsAndConditions": uidata.termsNconditions_1_t1,
...
alfabank.post('/lc-amend', function (req, res) {
	...
	var input = JSON.stringify(loc);
	//input.lCExpiryDate_t1 = "11/11/2017";


	//console.log("input OBJECT before calling AmendLetterOfCredit",input);
...
alfabank.post('/bg-amend', function (req, res) {
	...
	var msg = "Hi,\n I have amended the BG. Kindly approve. " + bog.bgNumber_t1 + " is amended";
...
alfabank.post('/bg-approve', function (getreqang, getresang) {
	var input = getreqang.body;
	//console.log("response of bg and bgid",input, input.bgNumber_t1, input.status);
...
alfabank.get('/meID', function (req, res) {
	res.send("100433");
});
...
alfabank.get('/get-customer-amendlc/:name', function (req, res) {
	var param = req.params.name;
...
alfabank.get('/get-customer-amendbg/:name', function (req, res) {
	var param = req.params.name;
	console.log("customer based amend records---->", param);
...
alfabank.get('/getCharges/:chargecode', function (req, res) {
